...
MWC_LIB1="${LOCALRT}/lib/slc3_ia32_gcc323/libDQMDTMonitorClient.so"
echo "Looking for the DTMonitorClient library... $MWC_LIB1"
if [ ! -f $MWC_LIB1 ]; then
    echo "Not Found! Will pick it up from the release area..."
    MWC_LIB1="/afs/cern.ch/cms/Releases/CMSSW/prerelease/${CMSSW_VERSION}/lib/slc3_ia32_gcc323/libDQMDTMonitorClient.so"
else 
    echo "Found!"
fi
...
sed -e "s/.portn/1972/g" -e "s/.host/${HOSTNAME}/g" -e "s/.pwd/${TEST_PATH}/g" -e "s/.libpath/${MWC_LIB}/g" .DTDQMClient.xml > DTDQMClient.xml 
